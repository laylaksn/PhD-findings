...
%true solution
phi=@(x,y) sin(pi*x)*sin(pi*y)+(x.^2-1)*(y.^2-1) + sin(2*pi*x).*sin(2*pi*y); 
...
epsilon = 1e-10; %this is the tolerence
...
%========================================================================
    ...
    for i=1:Kx %for each element in x
        ...
        for k=1:Ky   
            ...
            Fxy = (2)*(pi^2)*sin(pi*physx).*(sin(pi*physy')) -2*(physx.^2)-2*(physy'.^2)+4 + (8)*(pi^2)*sin(2*pi*physx).*(sin(2*pi*physy'));
...
%=========================================================================
    ...
    endTime = cputime;  
    enrichmentTime(Num) = endTime - startTime; 
...
fprintf('L2 errors for Example 5, i=3, using the LS SEM PGD, after three enrichments:')
fprintf('\n');
fprintf('\n'); 
fprintf('N =       ');
fprintf('%8d ', Nvalues);
...
disp(QL2v(end,end))
fprintf('\n');
fprintf('%d, %d and %d ADFPA iterations required at the first and second enrichment stages, respectively, for Example 5, i=3, using the LS SEM PGD, with epsilon = %6.2e\n', ...
...
for i = 1:3
    subplot(3,1,i);
    surf(xdomain, ydomain, Xphi(:,i) * Yphi(:,i)');   
