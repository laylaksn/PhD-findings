...
syms x y
% true solution
phi=@(x,y) sin(pi*x)*sin(pi*y) + sin(2*pi*x).*sin(2*pi*y);
U=@(x,y) - pi*cos(pi*x).*sin(pi*y)  - 2*pi*cos(2*pi*x).*sin(2*pi*y) ;
V=@(x,y) - pi*sin(pi*x).*cos(pi*y)   - 2*pi*sin(2*pi*x).*cos(2*pi*y);
...
% Number of elements in each direction - this is one for the LS SM PGD!
Kx=1; Ky=Kx;
            ...
            for i=1:Ky %for each element in y
                ...
                for i=1:Kx %for each element in x
                    ...
                    for k=1:Ky   
                        ...
                        Fxy = ep*((2)*(pi^2)*sin(pi*physx).*(sin(pi*physy')) + (8)*(pi^2)*sin(2*pi*physx).*(sin(2*pi*physy')));
                        Fxy = Fxy + b(1)*(pi*cos(pi*physx)*sin(pi*physy') +  2*pi*cos(2*pi*physx).*sin(2*pi*physy') );
                        Fxy = Fxy + b(2)*(pi*sin(pi*physx)*cos(pi*physy') + 2*pi*sin(2*pi*physx).*cos(2*pi*physy') );
...
%=========================================================================
                ...
                for term = 1:no_enrichment
                    ...
                    counter =0; 
                    R1 = randn(3*((N)*Kx-(Kx-1)),1); %returns a Nx by 1 matrix (column)
                    ...
                    QBEL2phi(Q,ei,bi)=L2errorphi;
                    if Q >2
                    if L2errorphi > QBEL2phi(2,ei,bi)
                        QBEL2phi(Q,ei,bi)  = QBEL2phi(2,ei,bi);
                    else 
                    end

                    else
                    end
                    QBEL2u(Q,ei,bi)=L2erroru;
                    QBEL2v(Q,ei,bi)=L2errorv;
    ...
    allb_cumcounter(bi,ei,:) = each_count_value;
     
    end

    % % results
    figure('Units', 'normalized', 'Position', [0.2 0.2 0.5 0.5]); 
    handles = semilogy(1:no_enrichment,QBEL2phi(:,:,bi), 'Linewidth', 2);
    legend('$\bar{\epsilon}= 1$','$\bar{\epsilon}= 0.1$','$\bar{\epsilon}= 0.01$', '$\bar{\epsilon}= 0.001$', 'Interpreter', 'latex','FontSize', 14 )
    grid on 
    ...
    title(sprintf('$\\mathbf{b} = (%.2f, %.2f)^T$', b(1), b(2)), ...
...
cuterrors = QBEL2phi(2:end,:,:);
max_abs_val = max(abs(cuterrors(:)));
...
A = squeeze(QBEL2phi(2,:,:));
A=A';

T = array2table(A, ...
    'VariableNames', {'epsilon bar = 1','epsilon bar = 0.1','epsilon bar = 0.01', 'epsilon bar = 0.001'}, ...  
    'RowNames', rowLabels);
fprintf('\n');
fprintf('L2 errors for φ for Example 7 using the LS SM PGD, after two enrichment with N=24:')
fprintf('\n');
disp(T)

Q_values = [1 2]; 
[bi_count, ei_count, ~] = size(allb_cumcounter);
A = zeros(bi_count, ei_count * numel(Q_values));
col = 1;
for ei = 1:ei_count
    for q = Q_values
        A(:, col) = allb_cumcounter(:, ei, q);
        col = col + 1;
    end
end
...
for ei = 1:ei_count
    for q = Q_values
        colNames{end+1} = sprintf('epsilon bar=%g, Q=%d', eps_values(ei), q);
    end
end
...
T = array2table(A, 'VariableNames', colNames, 'RowNames', rowLabels);
fprintf('\n');
fprintf('Number of ADFPA iterations at the first and second enrichments for Example 7 for the LS SM PGD, with N=24 ')
fprintf('\n');
fprintf('\n');
disp(T)
