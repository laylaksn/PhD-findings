...
%=======================================================================   
    ...
    % source term
    F=4.*pi^2.*(node.^2.*(1-node'.^2).^2+node'.^2.*(1-node.^2).^2).*sin(pi*(1-node.^2).*(1-node'.^2))+2.*pi*((1-node.^2)+(1-node'.^2)).*cos(pi.*(1-node.^2).*(1-node'.^2));      
...
fprintf('L2 errors for Example 4 using the LS SM PGD, for N=20 after ten enrichments:')
fprintf('\n');
fprintf('\n');
fprintf('L2 error φ   ');
