        ...
        for j = 1:N
            ...
            for m=1:N+1
                ...
                for n=1:N+1
                    ...
                    sumn = sumn + w(n)*( 2*pi^2*sin(pi*xco)*sin(pi*yco) - 2*(xco^2-1) - 2*(yco^2-1)  );
               end
                summ = summ + sumn*w(m);
            end
...
fprintf('L2 errors for Example 11, i = 2, using the MLS SM PGD, after two enrichments:')
fprintf('\n');
fprintf('\n');
fprintf('N =       ');
fprintf('%8d ', Nvalues);
...
fprintf('\n');
 fprintf('For N=20, and epsilon = %.e, %d and %d ADFPA iterations were used for the first and second enrichment, respectively.', ...
