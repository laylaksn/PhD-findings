...
% True solution 
phi=@(x,y) (x.^2-1)*(y.^2-1) + sin(pi*x)*sin(pi*y) + sin(2*pi*x)*sin(2*pi*y);
...
% %========================================================================
    ...
    for i=1:Kx %for each element in x
        ...
        for k=1:Ky   
            ...
            for ii = 1:N
                for jj = 1:N
                    Fxy(ii,jj) = 2 * pi^2 * sin(pi*physx(ii)) * sin(pi*physy(jj)) - 2*(physx(ii)^2-1) - 2*(physy(jj)^2-1)  + 8*pi^2 *sin(2*pi*physx(ii)) * sin(2*pi*physy(jj)); 
               end
            end 
...
set(gcf, 'Color', 'white')
legend('$N=8$','$N=12$','$N=16$','$N=20$','$N=24$' ,'interpreter','latex')
...
fprintf('L2 errors for Example 11, i = 3, using the MLS SEM PGD, after three enrichments:')
fprintf('\n');
fprintf('\n'); 
fprintf('N =       ');
fprintf('%8d ', Nvalues);
...
fprintf('\n');
 fprintf('For N=20, and epsilon = %.e, only %d, %d and %d ADFPA iterations were used for the first, second and third enrichments, respectively.', ...
    epsilon, each_count_value(1), each_count_value(2), each_count_value(3));
