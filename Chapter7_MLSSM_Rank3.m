        ...
        for j = 1:N
            ...
            for m=1:N+1
                ...
                for n=1:N+1
                    ...
                    sumn = sumn + w(n)*( 2*pi^2*sin(pi*xco)*sin(pi*yco) - 2*(xco^2-1) - 2*(yco^2-1)+ 8*pi^2*sin(2*pi*xco)*sin(2*pi*yco)  );
               end
                summ = summ + sumn*w(m);
            end
...
fprintf('\n');
 fprintf('For N=24, and epsilon = %.e, %d, %d and %d ADFPA iterations were used for the first, second and third enrichments, respectively.', ...
    epsilon, each_count_value(1), each_count_value(2), each_count_value(3));

fprintf('\n');
 fprintf('Time taken for the example with N=24 (in seconds):')
disp(enrichmentTime)
