% =====================================================================
% The section of this code involving computation of nodes, differentiation
...
syms x y
% true solution
phi=@(x,y) sin(pi*(1-x.^2).*(1-y.^2));
U=@(x,y) -2*pi.*x.*(y.^2-1).*cos(pi.*(x.^2-1).*(y.^2-1));
V=@(x,y) -2*pi.*y.*(x.^2-1).*cos(pi.*(x.^2-1).*(y.^2-1));
...
%tolerance
epsilon = 1e-8; %this is the tolerence
...
%========================================================================
    ...
    for i=1:Kx %for each element in x
        ...
        for k=1:Ky   
            ...
            Fxy=4.*pi^2.*(physx.^2.*(1-physy'.^2).^2+physy'.^2.*(1-physx.^2).^2).*sin(pi*(1-physx.^2).*(1-physy'.^2))+2.*pi*((1-physx.^2)+(1-physy'.^2)).*cos(pi.*(1-physx.^2).*(1-physy'.^2));      
        ...
        % for a global differentiation matrix
...
%=========================================================================
    ...
    for term = 1:no_enrichment
        ...
        while max(max( abs (R1*S1' - oldR* oldS') ) ) >epsilon
            ...
            M2_11 = (Rphi'*A*Rphi)*M + (Rphi'*M*Rphi)*A  ;
            M2_12 = (Rphi'*L'*Ru)*M ;
            M2_13 = (Rphi'*M*Rv)*L';
            M2_21 = (Ru'*L*Rphi)*M;
            M2_22 = (Ru'*A*Ru)*M + (Ru'*M*Ru)*A + (Ru'*M*Ru)*M;
            M2_23 = (Ru'*L'*Rv)*L - (Ru'*L*Rv)*L';
            M2_31 = (Rv'*M*Rphi)*L ;
            M2_32 = (Rv'*L*Ru)*L' - (Rv'*L'*Ru)*L;
            M2_33 = (Rv'*A*Rv)*M + (Rv'*M*Rv)*A + (Rv'*M*Rv)*M; 
            ...
            M1_11 = (Sphi'*M*Sphi)*A+(Sphi'*A*Sphi)*M ;
            M1_12 = (Sphi'*M*Su)*L' ;
            M1_13 = (Sphi'*L'*Sv)*M ;
            M1_21 = (Su'*M*Sphi)*L ;
            M1_22 = (Su'*M*Su)*A + (Su'*A*Su)*M + (Su'*M*Su)*M;
            M1_23 = (Su'*L*Sv)*L' - (Su'*L'*Sv)*L;
            M1_31 = (Sv'*L*Sphi)*M ;
            M1_32 = (Sv'*L'*Su)*L-(Sv'*L*Su)*L';
            M1_33 = (Sv'*A*Sv)*M+(Sv'*M*Sv)*M+(Sv'*M*Sv)*A;
        ...
        for i=1:Kx
            for j=1:Ky
                for num=1:N1
                    for m=1:N1
                        QL2phierror=QL2phierror+(ax(i+1)-ax(i))*(cy(j+1)-cy(j))/(2^(Kx))*w(m)*w(num)*(approxphi(m,num)-PHImesh(m,num))^2 ;
                        QL2uerror=QL2uerror+(ax(i+1)-ax(i))*(cy(j+1)-cy(j))/(2^(Kx))*w(m)*w(num)*(approxu(m,num)-Umesh(m,num))^2 ;
                        QL2verror=QL2verror+(ax(i+1)-ax(i))*(cy(j+1)-cy(j))/(2^(Kx))*w(m)*w(num)*(approxv(m,num)-Vmesh(m,num))^2 ;
                    end
                end
            end
        end
        ...
        QL2phi(Q,Num)=QL2phierror;
        QL2u(Q,Num)=QL2uerror;
        QL2v(Q,Num)=QL2verror;
    end
    endTime = cputime;  
...
N_values=Nvalues'; 
each_count_b(Q,:) = each_count_value;

allerrors = [QL2phierror;
    QL2uerror
    QL2verror];
...
set(gca,'fontsize',14);
set(gcf, 'Color', 'white')
handles = plot(1:no_enrichment,cumcounter_values, 'Linewidth', 2); 
grid on
xlabel('$Q$ enrichments','interpreter','latex','fontsize',14)
xlim([1 no_enrichment])
xticks(1:1:no_enrichment)
ylabel('Cumulative ADFPA iterations','interpreter','latex','fontsize',14)
 
fprintf('L2 errors for Example 4 using the LS SEM PGD, for N=20 after ten enrichments:')
fprintf('\n');
fprintf('\n');
fprintf('L2 error φ   ');
fprintf('%6.2e ', QL2phierror); 
fprintf('\n');
fprintf('L2 error u   ');
fprintf('%6.2e ', QL2uerror); 
fprintf('\n');
fprintf('L2 error v   ');
fprintf('%6.2e ', QL2verror); 
fprintf('\n');
fprintf('\n');
fprintf('For this example results were obtained with epsilon = %6.2e\n', ...
    epsilon);
fprintf('\n');

fprintf('Time taken for all ten enrichments with N=20 (in seconds):')
disp(enrichmentTime)