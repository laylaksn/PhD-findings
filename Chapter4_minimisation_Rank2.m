...
U=@(x,y) -pi*cos(pi*x).*sin(pi*y) -(2)*x.*(y.^2-1); 
V=@(x,y) - pi*sin(pi*x).*cos(pi*y) -(2)*y.*(x.^2-1);
...
%=======================================================================   
    ...
    node_transpose = node';
    % source term
    F=2*pi^2*sin(pi*node)*sin(pi*node_transpose) -2*(node.^2)-2*(node_transpose.^2)+4;
...
fprintf('L2 errors for Example 11, i = 3, using the MLS SM PGD, after two enrichments:')
fprintf('\n');
fprintf('\n');
fprintf('N =       ');
fprintf('%8d   ', Nvalues);
